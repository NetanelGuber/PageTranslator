...
class BergamotTranslatorWorker {
    ...
    loadTranslationModel({from, to}, buffers) {
        ...
        Object.assign(modelConfig, YAML.parse(`
            ...
            mini-batch-words: 256
            workspace: 32
            max-length-factor: 2.0
        `));

        const key = JSON.stringify({from,to});
        this.models.set(key, new this.module.TranslationModel(from, to, YAML.stringify(modelConfig), modelMemory, shortlistMemory, vocabs, qualityModel));
    }
    ...
    /**
     * Reports the WebAssembly linear-memory allocation. This is used only for
     * diagnostics in the extension's settings page; it contains no page data.
     */
    getMemoryStats() {
        return {
            wasmHeapBytes: this.module?.HEAP8?.buffer?.byteLength || 0,
